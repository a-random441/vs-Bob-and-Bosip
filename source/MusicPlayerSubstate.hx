...
class MusicPlayerSubstate extends MusicBeatSubstate
{
	...
	public function new(s:FlxPoint, unlockedSongs:Array<Bool>)
	{
		...
		for (i in 0...initSonglist.length)
		{
			...
			if (unlockedSongs[i]) {
				if (FileSystem.exists(SUtil.getStorageDirectory() + Paths.instcheck(data[0]))) {
				...
				if (FlxG.save.data.unlockedEX && FileSystem.exists(SUtil.getStorageDirectory() + Paths.instEXcheck(data[0]))) {
