		...
		for (i in 0...songs.length)
			{
				...
				if (ex) {
					if (songs[i].songName.toLowerCase() != 'gameover' || songs[i].songName.toLowerCase() == 'gameover' && FlxG.save.data.playedGO) { 
						if (FileSystem.exists(SUtil.getStorageDirectory() + Paths.instEXcheck(songs[i].songName.toLowerCase()))) {
				...
				if (FileSystem.exists(SUtil.getStorageDirectory() + Paths.instEXcheck(songs[i].songName.toLowerCase())) && presetPos) {
					if (songs[i].songName.toLowerCase() != 'gameover') {
